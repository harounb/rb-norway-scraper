...
describe("requestPage", () => {
  ...
  it("will throw an error if not provided a configuration object", () => {
    expect(() => {
      requestPages();
    }).toThrow();
    expect(() => {
  ...
  it("will return a promise that resolves with an array the contents got returns", () => {
    ...
    pLimit.mockImplementation(() => limit);
    got
      .mockReturnValueOnce(Promise.resolve("contents1"))
      .mockReturnValueOnce(Promise.resolve("contents2"))
      .mockReturnValueOnce(Promise.resolve("contents3"))
      .mockReturnValueOnce(Promise.resolve("contents4"));

    const requests = requestPages({
      sources: ["url1", "url2", "url3", "url4"],
      limit: 3
    }).then(([contents1, contents2, contents3, contents4]) => {
